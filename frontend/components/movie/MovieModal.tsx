    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative w-full max-w-4xl max-h-[90vh] bg-surface-card rounded-card overflow-hidden overflow-y-auto border border-border shadow-2xl"
        >
          {/* Close Button */}
          <button
            onClick={onClose}
            className="absolute top-4 right-4 z-10 w-10 h-10 bg-surface-raised/90 hover:bg-surface-elevated rounded-full flex items-center justify-center border border-border transition"
          >
          ...
          <div className="p-6 md:p-8 -mt-20 relative">
            {/* Title & Meta */}
            <h2 className="text-3xl md:text-4xl font-bold text-content-primary mb-2">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {movie.genres.map((genre, index) => {
                ...
                return (
                  <span
                    key={genreName || index}
                    className="px-3 py-1 bg-surface-raised rounded-full text-sm text-content-secondary border border-border"
                  >
                    {genreName}
                  </span>
                );
            ...
            {detail?.cast_members && detail.cast_members.length > 0 && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-content-primary mb-2">출연</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-surface-raised rounded-card border border-border">
              {/* 찜하기 버튼 */}
              <button
                ...
                className={`flex items-center justify-center space-x-2 px-6 py-3 rounded-md font-medium transition ${
                  isFavorited
                    ...
                    : "bg-surface-card hover:bg-surface-elevated text-content-secondary border border-border"
                }`}
              >
                <Heart
              ...
              <div className="flex items-center space-x-3">
                <span className="text-content-muted text-sm">내 평점:</span>
