...
## 키보드 단축키 패턴 (SearchAutocomplete)

`/` → 검색창 포커스, `Esc` → blur + 드롭다운 닫기

```tsx
// SearchAutocomplete.tsx — 기존 useEffect 클러스터에 추가
useEffect(() => {
...
- `tagName` 체크: input/textarea 안에서 `/` 입력은 그대로 통과
- `kbd` 힌트 뱃지: 검색어 없을 때 input 우측에 표시 (`sm` 이상, `pointer-events-none`)
